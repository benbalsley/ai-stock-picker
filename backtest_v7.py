...
############################################################
# Exit parameters (same as V5 for now)
############################################################
...
############################################################
# V7 Scoring: V5 logic, but looser gap + RS thresholds
############################################################

def score_ticker_v7(ticker: str, as_of: dt.date, df: pd.DataFrame, spy_df: pd.DataFrame):
    ...
    # RS vs SPY — slightly looser: allow up to ~1% underperformance
...
############################################################
# V7 Backtest: top 3 signals per day
############################################################

def run_backtest_v7(days_back=252, output_file="backtest_results_v7.csv", top_n=3):
    today = dt.date.today()
    ...
    print(f"[INFO] V7 Backtest window: last {days_back} trading days")
    ...
    if not universe:
        print("[ERROR] Universe is empty. Aborting V7 backtest.")
        return
    ...
    for d in signal_days:
        print(f"\n[DAY] {d} — computing V7 signals (top {top_n})...")
        ...
        for t, df in all_data.items():
            try:
                info = score_ticker_v7(t, d, df, spy_df)
            except Exception as e:
        ...
        if not day_signals:
            print(f"[DAY] {d}: No V7 signals passed filters.")
            continue
    ...
    if records:
        ...
        print(f"\n[RESULT] Backtest V7 written to {output_file}")
    else:
        print("\n[RESULT] No V7 trades generated.")
        return
...
if __name__ == "__main__":
    run_backtest_v7(days_back=252, top_n=3)
