...
############################################################
# V4 Scoring (same as V3)
############################################################

def score_ticker_v4(ticker: str, as_of: dt.date, df: pd.DataFrame, spy_df: pd.DataFrame):
    """
    V4 scoring: same as V3, but used in a multi-candidate (top N) selection.
    """
...
def build_position_size(entry_price, atr):
    """Risk per trade based on 1× ATR stop."""
    risk_dollars = ACCOUNT_EQUITY * RISK_PER_TRADE_PCT
    stop_distance = atr  # 1× ATR
...
############################################################
# V4 Backtest: top 2 signals per day
############################################################

def run_backtest_v4(days_back=252, output_file="backtest_results_v4.csv", top_n=2):
    today = dt.date.today()
    ...
    print(f"[INFO] V4 Backtest window: last {days_back} trading days")
    ...
    if not universe:
        print("[ERROR] Universe is empty. Aborting V4 backtest.")
        return
    ...
    for d in signal_days:
        print(f"\n[DAY] {d} — computing V4 signals (top {top_n})...")
        ...
        for t, df in all_data.items():
            try:
                info = score_ticker_v4(t, d, df, spy_df)
            except Exception as e:
        ...
        if not day_signals:
            print(f"[DAY] {d}: No V4 signals passed filters.")
            continue
    ...
    if records:
        ...
        print(f"\n[RESULT] Backtest V4 written to {output_file}")
    else:
        print("\n[RESULT] No V4 trades generated.")
        return
...
if __name__ == "__main__":
    run_backtest_v4(days_back=252, top_n=2)
