...
def score_ticker_v10(ticker: str, as_of: dt.date, df: pd.DataFrame, spy_df: pd.DataFrame):
...
############################################################
# ORB-style confirmation (V9 rule)
############################################################
...
def simulate_trade_for_day(signal_date, signal, df, trading_days):
    ...
    # V9 rule:
    # - if open > prev_high -> OK
    # - else if day's high > prev_high -> OK
    # - else skip trade
...
############################################################
# V10 Backtest: V9 logic over ~3 years, with per-year stats
############################################################

def run_backtest_v10(years=3, output_file="backtest_results_v10.csv", top_n=3):
    today = dt.date.today()
    ...
    print(f"[INFO] V10 Backtest window: last {years} years (~{days_back} trading days)")
    ...
    if not universe:
        print("[ERROR] Universe is empty. Aborting V10 backtest.")
        return
    ...
    fieldnames = [
        "signal_date", "trade_date", "trade_year", "ticker", "trend",
    ...
    for d in signal_days:
        print(f"\n[DAY] {d} — computing V10/V9 signals (top {top_n})...")
        ...
        for t, df in all_data.items():
            try:
                info = score_ticker_v10(t, d, df, spy_df)
            except Exception as e:
        ...
        if not day_signals:
            print(f"[DAY] {d}: No V10 signals passed filters.")
            continue
    ...
    if records:
        ...
        print(f"\n[RESULT] Backtest V10 written to {output_file}")
    else:
        print("\n[RESULT] No V10 trades generated.")
        return
    ...
    # Per-year stats (based on trade_date year)
    by_year = {}
    for r in records:
        y = r["trade_year"]
        by_year.setdefault(y, []).append(r)

    print("\n========== V10 YEAR-BY-YEAR BREAKDOWN ==========")
    for y in sorted(by_year.keys()):
        trs = by_year[y]
        pnl_y = [t["pnl_dollars"] for t in trs]
        wins_y = [p for p in pnl_y if p > 0]
        losses_y = [p for p in pnl_y if p <= 0]

        total_pnl_y = sum(pnl_y)
        total_ret_y = sum(t["pnl_pct"] for t in trs)
        win_rate_y = (len(wins_y) / len(trs) * 100.0) if trs else 0.0

        print(f"Year {y}:")
        print(f"  Trades:       {len(trs)}")
        print(f"  Win rate:     {win_rate_y:.2f}%")
        print(f"  Total PnL:    ${total_pnl_y:.2f}")
        print(f"  Total Return: {total_ret_y:.2f}% of account")
        if wins_y:
            print(f"  Avg Win:      ${np.mean(wins_y):.2f}")
        else:
            print("  Avg Win:      N/A")
        if losses_y:
            print(f"  Avg Loss:     ${np.mean(losses_y):.2f}")
        else:
            print("  Avg Loss:     N/A")
        print("------------------------------------------------")
    print("================================================")
...
if __name__ == "__main__":
    run_backtest_v10(years=3, top_n=3)
