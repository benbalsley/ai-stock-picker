...
############################################################
# Exit parameters (same as V7/V8 for now)
############################################################
...
############################################################
# V9 Scoring: same as V7/V8 (gap/RS/VWAP)
############################################################
...
############################################################
# ORB-style confirmation (looser): open OR intraday break
############################################################
...
############################################################
# V9 Backtest: top 3 signals per day + looser ORB
############################################################

def run_backtest_v9(days_back=252, output_file="backtest_results_v9.csv", top_n=3):
    today = dt.date.today()
    ...
    print(f"[INFO] V9 Backtest window: last {days_back} trading days")
    ...
    if not universe:
        print("[ERROR] Universe is empty. Aborting V9 backtest.")
        return
    ...
    for d in signal_days:
        print(f"\n[DAY] {d} — computing V9 signals (top {top_n})...")
        ...
        if not day_signals:
            print(f"[DAY] {d}: No V9 signals passed filters.")
            continue
    ...
    if records:
        ...
        print(f"\n[RESULT] Backtest V9 written to {output_file}")
    else:
        print("\n[RESULT] No V9 trades generated.")
        return
...
if __name__ == "__main__":
    run_backtest_v9(days_back=252, top_n=3)
