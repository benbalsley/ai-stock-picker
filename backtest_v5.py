...
############################################################
# Exit parameters (A: exit optimization infrastructure)
############################################################

# You can change these later to test different exits.
...
############################################################
# V5 Scoring: V4 logic + VWAP confirmation (B)
############################################################

def score_ticker_v5(ticker: str, as_of: dt.date, df: pd.DataFrame, spy_df: pd.DataFrame):
    """
    V5 scoring:
      - Long-only
    ...
    # (B) VWAP-style confirmation: close above proxy VWAP on signal day
    # Proxy VWAP ~ typical price = (H + L + C) / 3 for the last candle
    last_high = float(high.iloc[-1])
    last_low = float(low.iloc[-1])
    proxy_vwap = (last_high + last_low + last_close) / 3.0

    if last_close < proxy_vwap:
        # Price closed below typical price -> weaker candle -> skip
        return None
...
############################################################
# V5 Backtest: top 3 signals per day (C)
############################################################

def run_backtest_v5(days_back=252, output_file="backtest_results_v5.csv", top_n=3):
    today = dt.date.today()
    ...
    print(f"[INFO] V5 Backtest window: last {days_back} trading days")
    ...
    if not universe:
        print("[ERROR] Universe is empty. Aborting V5 backtest.")
        return
    ...
    for d in signal_days:
        print(f"\n[DAY] {d} — computing V5 signals (top {top_n})...")
        ...
        for t, df in all_data.items():
            try:
                info = score_ticker_v5(t, d, df, spy_df)
            except Exception as e:
        ...
        if not day_signals:
            print(f"[DAY] {d}: No V5 signals passed filters.")
            continue
        ...
        for signal in selected:
            ...
            if rec is None:
                print(f"[DAY] {d}, {signal['ticker']}: trade simulation failed.")
                continue
    ...
    if records:
        ...
        print(f"\n[RESULT] Backtest V5 written to {output_file}")
    else:
        print("\n[RESULT] No V5 trades generated.")
        return
...
if __name__ == "__main__":
    run_backtest_v5(days_back=252, top_n=3)
