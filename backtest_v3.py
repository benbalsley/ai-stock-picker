...
############################################################
# Market Regime (SPY + QQQ, scalar-only version)
############################################################
...
def get_market_regime(end_date: dt.date, lookback_days: int = 40):
    ...
    # Chop = very small intraday ranges for both
...
############################################################
# V3 Scoring (filters + RS)
############################################################

def score_ticker_v3(ticker: str, as_of: dt.date, df: pd.DataFrame, spy_df: pd.DataFrame):
    """
    V3 scoring:
      - Long-only
...
############################################################
# Main V3 Backtest Loop
############################################################

def run_backtest_v3(days_back=252, output_file="backtest_results_v3.csv"):
    today = dt.date.today()
    ...
    print(f"[INFO] V3 Backtest window: last {days_back} trading days")
    ...
    if not universe:
        print("[ERROR] Universe is empty. Aborting V3 backtest.")
        return
    ...
    for d in signal_days:
        ...
        for t, df in all_data.items():
            try:
                info = score_ticker_v3(t, d, df, spy_df)
            except Exception as e:
        ...
        if best_signal is None:
            print(f"[DAY] {d}: No V3 signals passed filters.")
            continue
    ...
    if records:
        ...
        print(f"\n[RESULT] Backtest V3 written to {output_file}")
    else:
        print("\n[RESULT] No V3 trades generated.")
        return
